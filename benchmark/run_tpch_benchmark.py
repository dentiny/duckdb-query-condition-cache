...
def benchmark_query(con, label: str, sql: str, repeat: int) -> dict:
    ...
    Steps:
      ...
      2. cache OFF, repeated   (warm OS baseline — true baseline, median of `repeat` runs)
...
def format_table(title: str, results: list[dict], sf: int, repeat: int) -> str:
    lines = [
        f"## {title} (SF={sf}, median of {repeat} warm runs)\n",
        f"| Query | Cold OS (ms) | Warm OS, no cache (ms) | Cache Build (ms) | Cache Hit (ms) | Speedup |",
        f"|-------|--------------|------------------------|------------------|----------------|---------|",
    ]
    for r in results:
        lines.append(
...
def plot_results(tpch_results: list[dict], dash_results: list[dict], sf: int, out_path: Path):
    ...
    fig.suptitle(f"QueryConditionCache Speedup — TPC-H SF={sf}", fontsize=14, fontweight="bold")
...
def main():
    ...
    out_file = args.out or str(WORKSPACE / "benchmark" / f"results_sf{args.sf}.md")
