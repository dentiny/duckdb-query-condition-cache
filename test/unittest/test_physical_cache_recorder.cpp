...
TEST_CASE("PhysicalCacheRecorder - non-contiguous vecs set bits exactly where observed", "[physical_recorder]") {
	...
	REQUIRE(local_entry.VectorPassesFilter(0, 0) == true);
	REQUIRE(local_entry.VectorPassesFilter(0, 1) == true);
	REQUIRE(local_entry.VectorPassesFilter(0, 3) == false);
	REQUIRE(local_entry.VectorPassesFilter(0, 4) == true);
	REQUIRE(local_entry.VectorPassesFilter(0, 7) == true);
}
...
TEST_CASE("PhysicalCacheRecorder - merging two task-local entries ORs observed and qualifying bits",
	...
	REQUIRE(destination->GetObservedVectorCount(0) == 3);
	REQUIRE(destination->GetObservedVectorCount(1) == 1);
	REQUIRE(destination->GetObservedVectorCount(2) == 1);
